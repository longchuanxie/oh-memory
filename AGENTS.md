...
After completing a logical subtask (implementing a function, fixing a bug, creating a file):
...
When making a design decision during work:
...
- Add entries to the `Changes` section (Created/Modified) with brief descriptions
...
- Keep it concise — this is a status dashboard, not a log
...
- Fill in all sections: Summary, Goal, Key Decisions, Changes, Insights, Next Steps
- Remove or minimize the `Detailed Log (Ephemeral)` section
...
- Update `By Topic` and `By Channel` sections if needed

### 3.4 Mark CHECKPOINT.md as Completed
- Set `status` to `"completed"`
- Update `lastUpdate` timestamp
- Clear `inProgress` and move remaining items as appropriate
...
> "Previous session appears to have terminated abnormally. Session: {sessionId}, Phase: {currentPhase}"

### 4.2 Check Git State
Run `git status` and `git diff --stat` to identify uncommitted changes. Report to the user:
- Branch name
- Number of modified/untracked files
- Last commit message

### 4.3 Present Recovery Options
Offer the user these 4 options:

1. **Continue** — Commit current changes and resume from CHECKPOINT.md state
2. **Review** — Show all changes (git diff), then let user decide
3. **Discard** — Revert all uncommitted changes (`git checkout .` + `git clean -fd`)
...
- When switching branches, check for existing sessions on that channel
