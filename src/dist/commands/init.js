...
function resolveTemplateDir(projectDir) {
...
function readTemplate(templateDir, templatePath) {
...
async function selectIDEs(ideFlag) {
    ...
    if (ideFlag && typeof ideFlag === "string") {
        ...
        return selected.filter((id) => (0, ide_detector_1.getIDEById)(id));
    }
...
function generateIDEConfig(ideId, projectDir, templateDir, vars, dryRun, force) {
    const created = [];
    const skipped = [];
    ...
    for (const file of mapping.files) {
        ...
        const { content } = (0, renderer_1.renderTemplate)(template, vars);
        if (dryRun) {
...
async function initCommand(flags, _positional) {
    ...
    if (interactive) {
        ...
        const ask = (prompt, defaultVal) => {
            return new Promise((resolve) => {
                rl.question(`${prompt} [${defaultVal}]: `, (ans) => {
    ...
    for (const file of FILES) {
        ...
        const template = readTemplate(templateDir, file.templatePath);
        const { content, unresolved } = (0, renderer_1.renderTemplate)(template, vars);
    ...
    if (!projectInfo.hasAgentsMd || force) {
        ...
        const { content: agentsContent, unresolved: agentsUnresolved } = (0, renderer_1.renderTemplate)(agentsTemplate, vars);
    ...
    if (selectedIDEs.length > 0) {
        ...
        selectedIDEs.forEach((id) => {
            const ide = (0, ide_detector_1.getIDEById)(id);
